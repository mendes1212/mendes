...
##### VARIAVEIS #####
    def VARIAVEIS(self):
        ...
        self.hakeando = "🔎 𝗔𝗴𝘂𝗮𝗿𝗱𝗲… 𝗶𝗱𝗲𝗻𝘁𝗶𝗳𝗶𝗰𝗮𝗻𝗱𝗼 𝗽𝗮𝗱𝗿𝗼̃𝗲𝘀"
        self.fim_do_sinal = "✅ 𝗦𝗶𝗻𝗮𝗹 𝗙𝗶𝗻𝗮𝗹𝗶𝘇𝗮𝗱𝗼, 𝗕𝗮𝘁𝗲𝘂 𝗮 𝗺𝗲𝘁𝗮? 𝗩𝗼𝗹𝘁𝗲 𝗮𝗺𝗮𝗻𝗵𝗮̃!"
...
#### SORTEANDO OS SINAIS DAS MINAS ####
    def SORTEIO_MINAS(self):
        print(f'''HORÁRIO ATUAL: {self.GET_HORA_ATUAL().format("HH:mm:ss")}''')
        print(" ")
        print("MINES SORTEIO")
        print(" ")
        cores = ['💎','⬛','⬛','⬛','⬛','⬛','⬛','⬛','⬛','⬛',
...
#### CRIANDO O BOT DO TELEGRAM ####
    ...
    def SINAL(self):
      ...
      sinal = self.bot.send_message(self.chat_id, f'''
🕰 𝗛𝗼𝗿𝗮́𝗿𝗶𝗼 𝗱𝗼 𝘀𝗶𝗻𝗮𝗹: {self.GET_HORA_ATUAL().add(minutes=0).format("HH:mm")}
✅ 𝗔𝗽𝗼𝘀𝘁𝗮: 5% da banca
💣 𝗠𝗶𝗻𝗮𝘀: {random.randint(2,5)}                                  
⏱ 𝗩𝗮𝗹𝗶𝗱𝗮𝗱𝗲: {self.GET_HORA_ATUAL().add(minutes=4).format("HH:mm")}
🎰 𝗠𝗮́𝘅𝗶𝗺𝗼 𝗱𝗲 𝘁𝗲𝗻𝘁𝗮𝘁𝗶𝘃𝗮𝘀: {random.randint(2,5)}
📊 𝗖𝗵𝗮𝗻𝗰𝗲 𝗱𝗲 𝗮𝗰𝗲𝗿𝘁𝗼: {random.randint(60,100)}%   
    ...
    def BOTAO(self):
        ...
        markup.add(types.InlineKeyboardButton(text=f"JOGUE AQUI", url="https://betbr.net/c-vMyr0wYj?lang=pt"))
        return markup


    def EDITAR_MESSAGE(self):
        
        message = self.bot.send_message(self.chat_id, self.fim_do_sinal)
...
#⏱𝙁𝙞𝙣𝙖𝙡𝙞𝙯𝙖𝙙𝙤 𝙖́𝙨: {self.GET_HORA_ATUAL().format("HH:mm:ss")}""", chat_id=self.chat_id, message_id=m_id)
    ...
    def ALERT_SINAL(self):
        ...
        message = self.bot.send_message(self.chat_id, self.hakeando)
        self.message_delete1 = False
        return message
